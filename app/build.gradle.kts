...
android {
    ...
    defaultConfig {
        ...
        versionCode = try {
            project.extensions.getByType<org.ajoberstar.grgit.Grgit>().log(mapOf("includes" to listOf("HEAD"))).size + 4029265
        } catch (e: Exception) {
            4029265 // fallback version code when git is not available
        }
...
fun generateFirebaseAppDistributionReleaseNotes(size: Int = 10) = buildString {
    ...
    try {
        project.extensions.getByType<org.ajoberstar.grgit.Grgit>().log(mapOf("maxCommits" to size)).forEach {
            append("* ").append(it.shortMessage.toString()).append('\n')
        }
    } catch (e: Exception) {
        append("* Build from non-git repository\n")
    }
}

afterEvaluate {
    android.applicationVariants.all { variant ->
        val packageUniversalApkTask = project.tasks.getByName("package${variant.name.replaceFirstChar { it.uppercaseChar() }}UniversalApk")

        val appDistributionTask = project.tasks.getByName("appDistributionUpload${variant.name.replaceFirstChar { it.uppercaseChar() }}")
